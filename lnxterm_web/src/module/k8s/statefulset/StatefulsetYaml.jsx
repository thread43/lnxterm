...
function StatefulsetYaml() {
  ...
  async function init() {
    try {
      const statefulset = storeStatefulset;

      setStateLoading(true);

      const response = await api.get_statefulset_yaml(statefulset);
  ...
  return (
    <>
      <Modal
        ...
        width={1000}
        open={storeStatefulsetYamlVisible}
      ...
      >
        <div className="MyContentHeader">
          <span className="MyContentHeaderTitle">
            {storeStatefulset.namespace} / {storeStatefulset.name}
          </span>
          <Space>
...
export default StatefulsetYaml;
