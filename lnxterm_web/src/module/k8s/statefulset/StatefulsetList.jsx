...
function StatefulsetList() {
  ...
  const storeStatefulsets = useSelector(store.getStatefulsets);
  const storeStatefulsetTableLoading = useSelector(store.getStatefulsetTableLoading);
  ...
  async function init(context) {
    ...
    try {
      dispatch(store.setStatefulsetTableLoading(true));
      ...
      const response2 = await api.get_statefulsets(cluster_id, namespace);
      dispatch(store.setStatefulsets(response2.data.data));
      ...
      dispatch(store.setStatefulsetTableLoading(false));
    }
  }
  ...
  async function changeCluster(value) {
    ...
    dispatch(store.setStatefulsets([]));

    try {
      dispatch(store.setStatefulsetTableLoading(true));

      const response = await api.get_statefulsets(cluster_id, namespace);
      dispatch(store.setStatefulsets(response.data.data));
  ...
  function getStatefulset(statefulset) {
    dispatch(store.setStatefulset(statefulset));
    dispatch(store.setStatefulsetDetailVisible(true));
  }

  function getStatefulsetYaml(statefulset) {
    dispatch(store.setStatefulset(statefulset));
    dispatch(store.setStatefulsetYamlVisible(true));
  }
    ...
    <>
      ...
      <div className="MyContentBlock">
        ...
        <Table
          rowKey="name"
          columns={columns}
          dataSource={storeStatefulsets}
          loading={storeStatefulsetTableLoading}
