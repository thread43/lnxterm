...
func GetStatefulsetYaml(response http.ResponseWriter, request *http.Request) {
	var err error
	...
	statefulset, err = clientset.AppsV1().StatefulSets(namespace).Get(context.Background(), name, meta_v1.GetOptions{})
	util.Raise(err)

	var statefulset2 []byte
	statefulset2, err = yaml.Marshal(statefulset)
	util.Raise(err)

	util.Api(response, 200, string(statefulset2))
}
