...
func GetServiceYaml(response http.ResponseWriter, request *http.Request) {
	var err error
	...
	service, err = clientset.CoreV1().Services(namespace).Get(context.Background(), name, meta_v1.GetOptions{})
	util.Raise(err)

	var service2 []byte
	service2, err = yaml.Marshal(service)
	util.Raise(err)

	util.Api(response, 200, string(service2))
}
